...
import { Menu, Bell, Search, Sun, Moon, HelpCircle } from 'lucide-react';
import { motion } from 'framer-motion';
import { Link, useNavigate } from 'react-router-dom';
import { useTheme } from '../../context/ThemeContext.jsx';
import { useAuth } from '../../context/AuthContext.jsx';

const LabHeader = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  const userName = user?.name || user?.firstName || user?.fullName || user?.username || 'Lab User';
  ...
  return (
    <header className="flex items-center justify-between p-4 bg-card border-b border-border sticky top-0 z-40">
      ...
      <div className="relative w-full max-w-md hidden sm:block">
          ...
          <input
              type="text"
              placeholder="Search tests, reports, inventory..."
      ...
      {/* Right Side Icons */}
      <div className="flex items-center space-x-4">
          {/* How To Use Link - Removed for Lab role */}
          ...
          {/* Notification Icon is now a Link */}
          <Link to="/lab/notifications" className="relative cursor-pointer">
